 ...
 *
 * Several keys are used for each session. Main key contains session data. Another key is created and used only for
 * session expire event. The session expire event will trigger the cleanup of the session. The third key is using
 * minutes as key index and contains a set of indexes of all sessions that expire in the minute preceding the minute
 * specified by index key. Following is the discussion of the algorithm sourced from Spring Session project.
 *
 * Redis has no guarantees of when an expired session event will be fired. In order to ensure expired session events are
 * processed in a timely fashion the expiration (rounded to the nearest minute) is mapped to all the sessions that
 * expire at that time. Whenever {@link #cleanExpiredSessions()} is invoked, the sessions for the previous minute are
 * then accessed to ensure they are deleted if expired. All nodes are running a task that does this monitoring, but only
 * the first node that polls Redis for a given key will be responsible for session expiration as we SMEMBERS and DEL on
 * the key within single Redis transaction.
 *
 * In some instances the {@link #cleanExpiredSessions()} method may not be not invoked for a specific time. For example,
 * this may happen when a server is restarted. To account for this, the expiration on the Redis session is also set. For
 * example, if none of the nodes was active during minute following session expiration, the check will not be done, and
 * sessions will silently expire.
 *
 * In stickiness scenario only the node owning the session will delete session on expire event. When using node
 * stickiness, the expire keys contain also node identifier. When listener receives event that this key expires, it
 * checks if key prefix matches the node's one. If it is the case, the session will be deleted. We need to handle also
 * the case when the owner node doesn't receive this notification (e.g. the node is down, there was network issue, Redis
 * servers are busy). For this reason we add `forced-expirations` key that is set one minute after the `expirations`
 * key. It has almost same semantics and logic, with the only difference being that the key is different and it is set
 * to expire one minute later.
 */
class NotificationExpirationManagement implements RedisExpirationStrategy {
  ...
  /**
   * After this number of milliseconds, forget that there was an issue with connectivity. 377 is 14th Fibonacci's number
   */
  private static final long RESET_RETRY_THRESHOLD = SECONDS.toMillis(377);

  /**
   * Exponential back-off is based on Fibonacci numbers - i.e. wait 0 seconds, 1 second, 1 second, 3 second etc.
   */
  ...
  /**
   * Cleans sessions that where left "hanging", i.e. they were not deleted when expire event was triggered. The hanging
   * session are expired on the node that first discovered the hanging session, and not necessarily the node that owned
   * the session.
   */
  ...
  /**
   * This class checks if there are any sessions which should have expired in previous minutes, but for which there were
   * no processed notification. If such sessions are found, the expiration notification is triggered using EXISTS
   * command.
   */
  ...
  final class ExpirationManagement {
    ...
    /**
     * Sets up all keys used during expiration management. Those are session key, key for session cleanup and optionally
     * clean for forced session cleanup when using sticky sessions.
     */
  ...
  /**
   * Creates Redis transaction that invokes atomically SMEMBERS and DEL on the given key
   *
   * @param key
  ...
  /**
   * Builds expirations key for the instant in milliseconds. Key is used to clean up sessions for which we didn't
   * receive expire event.
   *
   * @param instant
  ...
  /**
   * Builds forced expirations key for the instant in milliseconds. Key is used to clean up sessions for which we didn't
   * receive expire event as the owner node is down
   *
   * @param instant
   *          the instant for which to create key
   * @return key as byte array
  ...
  /**
   * Subscription task that listens to redis expiration notification. If an exception while listening to notifications
   * occurs, a exponential back-off strategy is applied. The task will retry to establish connection in increasing
   * interval until it succeeds or until maximum number of retries has been made.
   */
  ...
  public void reset() {
    if (expirationListener != null) {
      ...
      try {
        ...
        // Nothing to do because we make a reset when is down so it generates error for nothing
      }
